...
func (a *ProbeAttachment) getLogger(scopedLog *logrus.Entry) *logrus.Entry {
	return scopedLog.WithFields(logrus.Fields{
		probeType: a.ProbeType.AttachType(),
		probeName: a.ProbeName,
		funcName:  a.FuncName,
	})
}

// String returns a human-readable representation of the ProbeAttachment.
func (a *ProbeAttachment) String() string {
	return fmt.Sprintf("%s %s", a.ProbeType, a.ProbeName)
}
...
func (p *ProbeProg) LoadAndAttach() error {
	...
	for i, probe := range p.Probes {
		...
		if probe.IsKProbe() || probe.IsKRetProbe() {
			...
			return fmt.Errorf("Unknown probe type %s", probe.ProbeType)
		}
	}
	...
	for _, probe := range p.Probes {
		...
		case probe.IsKRetProbe():
			err = m.AttachKretprobe(probe.ProbeName, probe.fd)
		}

		if err != nil {
			return fmt.Errorf("unable to attach to %s: %s",
				probe.String(), err)
		}
