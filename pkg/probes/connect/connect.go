...
func (c *connectProbe) OnAttach() error {
	...
	go func() {
		...
		for {
			...
			// TODO: Handle connectionless sockets. From connect(2):
			//       "connectionless protocol sockets may use
	...
	go func() {
		...
		for {
			...
			case api.ProbeEnterExecute:
				...
				if context == nil {
					log.Error("BUG: nil ProcessContext")
					continue
				}
				context.AddExecveEvent(strings.TrimRight(string(event.Command[:taskCommLen]), "\x00"))
			case api.ProbeEnterExit:
				process.Cache.Delete(pid)
			default:
				log.Warn("Failed to handle commEvent type %s", event.Typ)
			}
		}
	}()
		...
		ProbeProg: api.ProbeProg{
			...
			Probes: []api.ProbeAttachment{
				...
				},
				{
					ProbeType: api.ProbeEnterExit,
					FuncName:  "syscall__exit",
					ProbeName: bcc.GetSyscallFnName("exit_group"),
				},
				// TODO: sys_kill()
			},
		},
